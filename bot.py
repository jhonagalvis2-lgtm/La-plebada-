...
def preguntar_claude(uid, texto):
    ...
    r = requests.post(
        ...
        json={"model": "claude-haiku-4-5-20251001", "max_tokens": 512, "system": SYSTEM_PROMPT, "messages": historial[uid][-10:]}
    )
    ...
    if q.data == "premium":
        if es_premium(uid):
            ...
            await q.edit_message_text(
                ...
                "• 🤖 IA sin límites de preguntas\n"
                "• 📚 Lecciones exclusivas\n"
                "• 💬 Acceso a grupo VIP\n\n"
                "📱 *Cómo pagar:*\n"
                f"1. Envía {PRECIO} por Nequi al número que te dará el admin\n"
                f"2. Toca el botón de abajo para notificar tu pago\n"
                f"3. En menos de 1 hora te activamos el acceso ✅\n\n"
                f"Tu ID: `{uid}` _(cópialo para enviarlo con el comprobante)_",
                parse_mode="Markdown",
    ...
    if q.data == "notificar_pago":
        ...
        await context.bot.send_message(
            ...
            text=f"💰 *Nuevo pago reportado!*\n\n"
                 ...
                 f"📱 Username: @{username}\n"
                 f"🆔 ID: `{uid}`\n\n"
                 f"Para activar escribe:\n`/activar {uid}`",
            parse_mode="Markdown"
        )
        await q.edit_message_text(
            "✅ *Pago notificado!*\n\n"
            "El admin revisará tu pago y te activará el acceso en menos de 1 hora.\n\n"
            "Recuerda enviar el comprobante de Nequi al admin 📸",
            parse_mode="Markdown", reply_markup=volver()
        )
        return
